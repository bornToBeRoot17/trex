...
def generate_graphs(values_clf, prefix_name_save):
    ...
    for clf in values_clf.classifier.unique():
        ...
        ax.set_title(clf + ' results',  fontdict={'fontsize': 24, 'fontweight':'bold'})
        ax.set_xlabel('Extractors',     fontdict={'fontsize': 16}) #, 'fontweight':'bold'})
        ax.set_ylabel('Percentage (%)', fontdict={'fontsize': 16}) #, 'fontweight':'bold'})
        ax.set(ylim=(0, 100))
        #ax.set_yticks([0.1,0.2,0.3,0.4])

        cm = 1/2.54
        figure = plt.gcf()
        figure.set_size_inches(13*cm,16*cm)
        ax.legend(fontsize=12) #, prop={'size': 9})
        ...
        #plt.show()
        plt.tight_layout()
        plt.savefig(graph_path + '/' + clf+'_'+prefix_name_save+'.pdf') #, bbox_inches='tight', dpi=100)
        plt.clf()
