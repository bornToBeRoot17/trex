...
def write_training_scikit(class_name, hist, method, filename, class_img, label, dim_images):
    if hist == None:
        return
    sci_path=get_sckit_filepath(dim_images,out=1)
    f=open(sci_path+"/"+method+"/"+filename, "a+")
...
def get_definitions(section,option,dim_images):
    c = configparser.ConfigParser()
    c.read("./definitions_"+dim_images+".ini")
...
def get_applications(dim_images):
    apps = {}
    for i in range(1, int(get_definitions("Applications","number",dim_images))+1):
        app,app_path=get_definitions("Applications","app"+str(i),dim_images).split("=")
        apps[app]=app_path
    return apps
